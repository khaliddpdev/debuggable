...
function Debuggable(options) {
    var defaults = {
        ...
        _options = options ? {
            debugLevel: options.debugLevel || defaults.debugLevel,
            debugTag: options.debugTag || defaults.debugTag
        } : defaults;
...
for (var debugLevel in DEBUG){
    if (DEBUG.hasOwnProperty(debugLevel)){
        Debuggable[debugLevel] = DEBUG[debugLevel];
    }
}
